...
Конфигурация provider принадлежит конкретному `JamRound`. У каждого раунда могут быть свои itch.io JAM ID и alias MyIndie; глобальные идентификаторы provider между раундами не используются.
...
`registrationStartsAt` — необязательное поле. Если оно задано, регистрация открыта с этого момента до `startsAt`, не включая сам момент старта раунда. Если поле отсутствует, регистрация считается недоступной. Главная страница выбирает активный раунд отдельно от раунда с открытой регистрацией.

Для выбора используются `getActiveRound`, `getRegistrationOpenRound` и существующий `getFinishedRounds`. Первые две функции возвращают независимые сущности, поэтому текущий раунд может оставаться активным, пока регистрация на следующий уже открыта.
